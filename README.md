...
async function bootstrap(userCount: number, assignmentCount: number) {
  ...
  for (let i = 0; i < userCount; i++) {
    await assignmentMatcher.addUser({
      id: `user_${i}`,
      tags: ['tag1', 'tag2', `unique_skill_${i % 10}`], // Varied tags
    });
  }
  ...
  for (let i = 0; i < assignmentCount; i++) {
    await assignmentMatcher.addAssignment({
      id: `assignment_${i}`,
      tags: ['tag1', `unique_skill_${i % 10}`], // Ensure some matches
      createdAt: new Date().getTime() + i, // Stagger creation time slightly
    });
  }
  console.log('Setup complete.');

  // --- Performance Measurement for Matching ---
  performance.mark('match-start');
  await assignmentMatcher.matchUsersAssignments();
  performance.mark('match-end');
  const matchMeasure = performance.measure('Matching Process', 'match-start', 'match-end');
  console.log(`Matching execution time: ${matchMeasure.duration.toFixed(2)}ms`);

  // --- Performance Measurement for Retrieving Assignments ---
  performance.mark('current-assignment-start');
  // Let's check assignments for a few users
  for (let j = 0; j < Math.min(userCount, 5); j++) {
    await assignmentMatcher.getCurrentAssignmentsForUser(`user_${j}`);
  }
  performance.mark('current-assignment-end');
  const currentTaskMeasure = performance.measure(
    'Retrieving User Assignments (for 5 users)',
    'current-assignment-start',
    'current-assignment-end'
  );
  console.log(`Current assignment retrieval time: ${currentTaskMeasure.duration.toFixed(2)}ms`);

  // Example: Log assignments for the first user
  if (userCount > 0) {
    const firstUserAssignments = await assignmentMatcher.getCurrentAssignmentsForUser('user_0');
    console.log(`User 'user_0' has ${firstUserAssignments.length} assignments.`);
    // firstUserAssignments.forEach(asgn => console.log(`  - Assignment ID: ${asgn.id}, Tags: ${asgn.tags.join(', ')}`));
  }
...
Summary:
    ...
    Matching Duration: ${matchMeasure.duration.toFixed(2)}ms
    Retrieval Duration (5 users): ${currentTaskMeasure.duration.toFixed(2)}ms`
  );
