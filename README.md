...
**Shift swaps between workers.** `rankSwapPartners(input, shiftInstanceId, fromEmployeeId, roster, { trades, tradesFrom })` answers "who could take my shift, and what could I take back?". It is `rankCandidates` asked about a shift somebody is *in*: the proposer is taken off it first (asked with them still on it, a shift at its `maxEmployees` reads "no room" for everybody), and ordering, site fields and blockers are `rankCandidates`' own. With `trades: true` each partner carries the shifts of theirs that make a lawful trade — judged with **both** people off the shift they give up, in **both** directions, on one model build. `checkSwap(input, { fromEmployeeId, fromShiftInstanceId, toEmployeeId, toShiftInstanceId }, roster)` judges one proposed give-away (`toShiftInstanceId: null`) or trade the same way and returns `{ fits, blockers }`, each blocker naming the person it applies to. Neither judges rules that span a whole shift or period (tag counts and ratios, aggregates): re-check the resulting roster with `checkCompliance` before applying it.

**Skill cover.** `diagnoseInfeasibility(input).tagCover` lists, per skill the roster asks for, its demand against the people who hold it: `shifts` asking for it, `seats` (people-shifts), `holders` (anyone holding it on a day it is wanted), `shortDates` — days it asks for more holders than are free (holding it that day, not on time off, inside their contract; one person counts once a day, so an early and a late both wanting a key-holder need two) — and `tightDates`, days with no spare. `kind` is `'mix'` for `tagRequirements`/`tagRatios` and `'everyone'` for `requiredTags`. It is informational and never makes the report infeasible on its own. Each `RepairCandidate` from `rankCandidates` and `rankSwapPartners` carries `tagsHeld`: which of the shift's `tagRequirements` that person holds on its date, at the level asked for — declared qualifications only, so the ranking stays profiling-free.
